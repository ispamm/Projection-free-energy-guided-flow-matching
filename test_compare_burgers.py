...
from physics_engine import BurgersEquationResidualsFull
...
def parse_args():
    ...
    parser.add_argument("--config_path", type=str, default="configs/burgers1d_white.yml", help="config.yml path for the experiment")
    parser.add_argument("--ckpt_path", type=str, default="logs/burgers1d_white_test/latest.pt", help="Checkpoint (.pt) path for the pretrained model")
...
def main():
    ...
    if hasattr(config, 'nu'):
        nu_default = float(getattr(config, 'nu'))
    elif isinstance(config, dict) and 'nu' in config:
        nu_default = float(config['nu'])
    else:
        data_cfg = None
        ...
        if data_cfg is not None:
            if hasattr(data_cfg, 'nu'):
                nu_default = float(getattr(data_cfg, 'nu'))
    ...
    def log_periodic_metrics(processed_samples: int):
        ...
        periodic_table = wandb.Table(columns=["Method", "Success Rate (%)", "Generated Samples", "Speed (sec/sample)", "Physical Residual (MAE)", "IC Error", "BC Error", "CL Error", "MMSE", "SMSE", "SampleMSE"])
        for name, tracker in trackers.items():
            ...
            if (not failed_with_nan) and success_count == args.n_samples:
                ...
                mask_bc = torch.zeros_like(u_exact_ref, dtype=torch.bool)
                mask_bc[:, 0, :] = True
                mask_bc[:, -1, :] = True

                ce_ic = torch.nn.functional.mse_loss(u_pred_all[:, :, 0], u_exact_ref[:, :, 0]).item()
                ce_bc = torch.nn.functional.mse_loss(u_pred_all[mask_bc], u_exact_ref[mask_bc]).item()
            ...
            periodic_table.add_data(name, success_rate, success_count, speed, residual, ce_ic, ce_bc, ce_cl, mmse, smse, sample_mse)
    ...
    for i in range(args.n_samples):
        ...
        physics_rules = BurgersEquationResidualsFull(data=u_exact_i, nx=dims[0], nt=dims[1], nu=nu_exact, spatial_domain=[0, 1]) 


        # Original PCFM residuals
        # hfunc_pcfm = Residuals(
        #     data=u_exact_all, # PCFM vuole l'intero dataset per estrarre i target
        #     x=x_grid, 
        #     t_grid=t_grid,
        #     nu=config['data'].get('nu', 0.01)
        # )

        # # Wrapper per renderla compatibile con hfunc(u)
        # def hfunc(u):
        #     u_flat = u.flatten()
        #     ic = hfunc_pcfm.ic_residual(u_flat)
        #     bc = hfunc_pcfm.bc_residual_burgers(u_flat) # Solo BC e Massa
        #     return torch.cat([ic.flatten(), bc.flatten()])



        def guidance_hfunc(u_flat):
            ic = physics_rules.ic_residual(u_flat)
            #mass = physics_rules.mass_residual(u_flat)
            
            # Moltiplichiamo per dx^2 per disinnescare l'esplosione sul rumore
            bc = physics_rules.bc_residual(u_flat)
            pde = physics_rules.pde_residual(u_flat) * (physics_rules.dx ** 2) 
        ...
        def eval_hfunc(u_flat):
            ic = physics_rules.ic_residual(u_flat)
            bc = physics_rules.bc_residual(u_flat)
            pde = physics_rules.pde_residual(u_flat) # NESSUNA SCALA QUI!
            return torch.cat([ic, bc, pde], dim=0)

        hfunc = guidance_hfunc

        ic_err = physics_rules.ic_residual(u_exact_i.flatten()).abs().mean().item()
        #mass_err = physics_rules.mass_residual(u_exact_i.flatten()).abs().mean().item()
    ...
    wandb_log_dict = {}
    columns = ["Method", "Success Rate (%)", "Generated Samples", "Speed (sec/sample)", "Physical Residual (MAE)", "IC Error", "BC Error", "CL Error", "MMSE", "SMSE", "SampleMSE"]
    ...
    for name, tracker in trackers.items():
        ...
        results_table.add_data(name, success_rate, success_count, speed, residual, ce_ic, ce_bc, ce_cl, mmse, smse, sample_mse)
