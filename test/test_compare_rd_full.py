...
from physics_engine import ReactionDiffusionResidualsFull
...
def parse_args():
	...
	parser.add_argument("--config_path", type=str, default="configs/rd1d_white.yml", help="config.yml path for the experiment")
	parser.add_argument("--ckpt_path", type=str, default="logs/rd1d_white/latest.pt", help="Checkpoint (.pt) path for the pretrained model")
...
def main():
	...
	if cond_type != "ic":
		raise NotImplementedError(f"Unsupported cond_type for rd full: {cond_type}")
	...
	# RD defaults from generator
	rho_default = 0.01
	nu_default = 0.005

	if hasattr(test_set, "file"):
		rho_default = float(test_set.file.attrs.get("rho", rho_default))
		nu_default = float(test_set.file.attrs.get("nu", nu_default))
	...
	if hasattr(test_set, "file") and "x" in test_set.file and "t" in test_set.file:
		x_grid = torch.from_numpy(test_set.file["x"][:]).to(device=device, dtype=torch.float32)
		t_grid = torch.from_numpy(test_set.file["t"][:]).to(device=device, dtype=torch.float32)
	else:
		x_grid = torch.linspace(0, 1, dims[0], device=device)
		t_grid = torch.linspace(0, 0.99, dims[1], device=device)

	print("3. Generation of initial noise (u0)...")
	grid = make_grid(
		dims,
		device,
		start=(float(x_grid[0].item()), float(t_grid[0].item())),
		end=(float(x_grid[-1].item()), float(t_grid[-1].item())),
	)
	...
	def compute_component_errors(u_pred, u_true, physics_rules_eval, mass_rules):
		u_pred_flat = u_pred.flatten()
		u_true_flat = u_true.flatten()

		ic_error = torch.nn.functional.mse_loss(u_pred[:, :, 0], u_true[:, :, 0]).item()

		gL_pred = physics_rules_eval.bc_left_flux(u_pred_flat)
		gL_true = physics_rules_eval.bc_left_flux(u_true_flat)
		bc_left_error = torch.nn.functional.mse_loss(gL_pred, gL_true).item()

		gR_pred = physics_rules_eval.bc_right_flux(u_pred_flat)
		gR_true = physics_rules_eval.bc_right_flux(u_true_flat)
		bc_right_error = torch.nn.functional.mse_loss(gR_pred, gR_true).item()
		...
		mass_error = mass_rules.mass_residual_rd(u_pred_flat)[1:].abs().mean().item()
	...
	print("4. Iterative Generation (full PDE residual guidance)...")
	sampler = FFM_sampler(model, model.gp)
	...
	for i in range(args.n_samples):
		...
		physics_rules_guidance = ReactionDiffusionResidualsFull(
			data=u_exact_i,
			nx=dims[0],
			nt=dims[1],
			nu=nu_default,
		...
		def hfunc(u_flat):
			return physics_rules_guidance.full_residual(u_flat)

		eval_hfunc = hfunc

		ic_err = physics_rules_guidance.ic_residual(u_exact_i.flatten()).abs().mean().item()
		mass_err = mass_rules.mass_residual_rd(u_exact_i.flatten())[1:].abs().mean().item()
		...
		# cond_type is IC-only for RD configs in this repository
		...
		if method_active.get("ECI", False):
			...
			if not deactivate_if_nan("ECI", res_eci, i):
				record_component_errors("ECI", compute_component_errors(u_eci, u_exact_i, physics_rules_eval, mass_rules))
		...
		if method_active.get("DiffusionPDE", False):
			...
			if not deactivate_if_nan("DiffusionPDE", res_diffpde, i):
				record_component_errors("DiffusionPDE", compute_component_errors(u_diffpde, u_exact_i, physics_rules_eval, mass_rules))
		...
		if method_active.get("DFlow", False):
			...
			if not deactivate_if_nan("DFlow", res_dflow, i):
				record_component_errors("DFlow", compute_component_errors(u_dflow, u_exact_i, physics_rules_eval, mass_rules))
		...
		if method_active.get("Vanilla", False):
			...
			if not deactivate_if_nan("Vanilla", res_vanilla, i):
				record_component_errors("Vanilla", compute_component_errors(u_vanilla, u_exact_i, physics_rules_eval, mass_rules))
		...
		if method_active.get("PROFlow", False):
			...
			if not deactivate_if_nan("PROFlow", res_proflow, i):
				record_component_errors("PROFlow", compute_component_errors(u_proflow, u_exact_i, physics_rules_eval, mass_rules))
		...
		if method_active.get("PCFM", False):
			...
			if not deactivate_if_nan("PCFM", res_pcfm, i):
				record_component_errors("PCFM", compute_component_errors(u_pcfm, u_exact_i, physics_rules_eval, mass_rules))
		...
		for g in args.gamma_list:
			...
			if method_active.get(tracker_key, False):
				...
				u_ours, _ = sampler.continuous_guided_sample(
					u0_i, args.n_steps, hfunc, gamma_max=g,
					final_refinement=False, refinement_steps=20,
					refinement_lr=0.1, gamma_schedule="sine"
				)
				res_ours = compute_physical_residual(u_ours, hfunc)
				end_t = time.time()
				if not deactivate_if_nan(tracker_key, res_ours, i):
					record_component_errors(tracker_key, compute_component_errors(u_ours, u_exact_i, physics_rules_eval, mass_rules))
