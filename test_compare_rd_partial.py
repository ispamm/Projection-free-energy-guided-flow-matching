...
def main():
	...
	if cond_type != "ic":
		raise NotImplementedError(f"Unsupported cond_type for rd partial: {cond_type}")
	...
	for i in range(args.n_samples):
		...
		physics_rules_constraints = Residuals(
			data=u_exact_i,
			x=x_grid,
			t_grid=t_grid,
			nx=dims[0],
		...
		physics_rules_eval = ReactionDiffusionResidualsFull(
			data=u_exact_i,
			nx=dims[0],
			nt=dims[1],
			nu=nu_default,
		...
		def hfunc(u_flat):
			return physics_rules_constraints.full_residual_rd(u_flat)

		eval_hfunc = hfunc

		ic_err = physics_rules_constraints.ic_residual(u_exact_i.flatten()).abs().mean().item()
		mass_err = physics_rules_constraints.mass_residual_rd(u_exact_i.flatten())[1:].abs().mean().item()
